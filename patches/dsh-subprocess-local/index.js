...
*/
function readWindowsProcessTable(internals) {
	try {
		const script = "Get-CimInstance Win32_Process | ForEach-Object { '{0}|{1}|{2:O}' -f $_.ProcessId, $_.ParentProcessId, $_.CreationDate }";
		const text = internals.exec("powershell.exe", [
			"-NoLogo",
			"-NoProfile",
			"-NonInteractive",
			"-Command",
		...
		for (const line of text.split(/\r?\n/)) {
			...
			const pid = Number(row.slice(0, first));
			const parentPid = Number(row.slice(first + 1, second));
			const started = row.slice(second + 1).trim();
			if (!Number.isSafeInteger(pid) || pid <= 0 || !Number.isSafeInteger(parentPid) || started.length === 0) continue;
			table.set(pid, { pid, parentPid, started });
		}
		return table;
...
function windowsTree(internals, rootPid) {
	const table = readWindowsProcessTable(internals);
	if (table === void 0 || !table.has(rootPid)) return [{ pid: rootPid, started: `pid:${rootPid}` }];
	const byParent = /* @__PURE__ */ new Map();
	...
	visit(table.get(rootPid));
	return result.length > 0 ? result : [{ pid: rootPid, started: `pid:${rootPid}` }];
}
var WindowsProcessInspector = class {
	...
	processTable() {
		const now = Date.now();
		if (this.tableCache !== void 0 && now - this.tableCacheAt < 100) return this.tableCache;
		this.tableCache = readWindowsProcessTable(this.internals);
		this.tableCacheAt = now;
		return this.tableCache;
	}
	signalGroup(pgid) {
		if (pgid <= 0) return;
		try {
			taskkillProcessTree(pgid);
		} catch (_taskkillUnavailable) {}
	}
	signalProcess(identity, signal) {
		if (!this.isAlive(identity)) return;
		try {
			this.internals.kill(identity.pid, signal);
		} catch (_unsupportedWindowsSignal) {
			try {
				taskkillProcessTree(identity.pid);
			} catch (_taskkillUnavailable) {}
		}
	}
	foregroundPgid(shellPid) {
	...
	processTree(rootPid) {
		return windowsTree(this.internals, rootPid);
	}
	processSession(_sessionId) {
		return [];
	}
	isAlive(identity) {
		...
		if (table !== void 0) {
			...
			if (identity.started === entry.started || identity.started === `pid:${entry.pid}`) return true;
			return identity.started === void 0;
		}
		try {
			this.internals.kill(identity.pid, 0);
			return true;
		} catch (_processAbsent) {
			return false;
		}
	}
};
...
var LocalTerminalHandle = class {
	...
	*/
	...
	async inspectForeground() {
		if (process.platform !== "win32") this.descendants();
	...
	*/
	...
	descendants() {
		...
		const rootVerified = this.rootIdentity !== void 0 && root !== void 0 && (root.started === this.rootIdentity.started || root.started === `pid:${this.rootIdentity.pid}` || this.rootIdentity.started === `pid:${root.pid}`);
	...
	async stopShell() {
		if (process.platform === "win32") {
			if (!this.exited) {
				if (this.rootIdentity !== void 0) this.signalMembers([this.rootIdentity], "SIGKILL");
				else try {
					this.terminal.kill();
