...
function defaultShellPath() {
	...
	for (const candidate of WINDOWS_BASH_CANDIDATES) try {
	...
	async initialize(signal) {
		...
		try {
			...
			if (result.waitReason === "session_exit") {
				const status = result.sessionStatus.kind === "exited" ? ` (exitCode=${result.sessionStatus.exitCode}, signal=${result.sessionStatus.signal})` : "";
				throw new Error(`PTY shell exited during startup${status}; captured output:\n${result.viewport || "(no output)"}`);
			}
...
function spawnArgv(ctx, config, policy) {
	...
	if (policy.mode === "danger-full-access" || config.sandbox === false) return argv;
...
var BashTerminalBackend = class {
	...
	async spawn(spec) {
		...
		try {
			...
			throw new Error(`terminal-bash startup failed for argv=${JSON.stringify(argv)} cwd=${JSON.stringify(spec.cwd ?? policy.workspaceRoot)}: ${message}`, { cause: error });
		}
	}
};
